...
const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://localhost:5432/weekly_updates';
...
async function seedDatabase() {
  ...
  try {
    ...
    const usersResult = await client.query(usersQuery, [passwordHash, passwordHash, passwordHash]);
      ...
      for (let i = 0; i < updateCount; i++) {
        ...
        if (Math.random() > 0.7) {
          await client.query(
            ...
            [
              perfId,
              'Delayed integration testing due to environment issues'
            ]
          );
        }
        ...
        await client.query(
          ...
          [
            engagementId,
            'Client is pleased with progress and communication'
          ]
        );
